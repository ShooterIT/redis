...
test {Release the thread of sending rdb when close replica's connection} {
    start_server {tags {"repl"}} {
        set replica [srv 0 client]
        set replica_host [srv 0 host]
        set replica_port [srv 0 port]
        start_server {overrides {rdb-bulk-send-delay 1000000}} {
            set master [srv 0 client]
            set master_log [srv 0 stdout]
            set master_host [srv 0 host]
            set master_port [srv 0 port]

            # Write 1M data, sending all rdb will cost several seconds
            ...
            after 100
            # Send RDB by thread and release successfully
            assert {[log_file_matches $master_log "*start*thread*send*${replica_host}*${replica_port}*"]}
            assert {[log_file_matches $master_log "*thread*${replica_host}*${replica_port}*released*killing*"]}
        }
    }
}
