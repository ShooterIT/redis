...
sds asmDumpActiveImportTask(void) {
    ...
    /* For slave, dump the master task. */
...
int asmReplicaHandleMasterAsmTask(sds task_info) {
    ...
    /* If the task information is empty, it means the master finishes the task, the replica
     * should check the slot ownership to decide to raise completed or failed event. */
    if (!task_info || sdslen(task_info) == 0) {
        asmTask *master_task = asmManager->master_task;
        serverAssert(master_task->operation == ASM_IMPORT);
        if (master_task && master_task->state != ASM_DONE && master_task->state != ASM_FAILED) {
            /* Check if the slots are owned by the master. */
            int owned_by_master = 1;
            for (int i = 0; i < master_task->slot_ranges->num_ranges; i++) {
                slotRange *sr = &master_task->slot_ranges->ranges[i];
            ...
            if (owned_by_master) {
                master_task->state = ASM_DONE;
                asmNotifyStateChange(master_task, ASM_EVENT_IMPORT_COMPLETED);
            } else {
                master_task->state = ASM_FAILED;
                asmNotifyStateChange(master_task, ASM_EVENT_IMPORT_FAILED);
            }
        }
        return C_OK;
    }
